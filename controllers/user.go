...
type UserHandler interface {
	...
	GetUserCoins(*gin.Context)
}

type userHandler struct {
	repo config.UserRepository
}
...
func (h *userHandler) SignInUser(ctx *gin.Context) {
	...
	if isTrue := comparePassword(dbUser.Password, user.Password); isTrue {
		...
		ctx.JSON(http.StatusOK, gin.H{"msg": "Successfully SignIN"})
		return
	}
...
func (h *userHandler) GetUserCoins(ctx *gin.Context) {

	userStr := ctx.Param("user")
	userID, _ := strconv.Atoi(userStr)
	if products, err := h.repo.GetUserCoins(userID); err != nil {
